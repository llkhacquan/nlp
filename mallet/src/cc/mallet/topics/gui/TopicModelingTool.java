...
public class TopicModelingTool {
	...
	JFrame frame, frame2;
	JButton openButton, trainButton, advancedButton, stopChooseButton,
			outputDirButton;
	JButton clearButton;
	JTextArea log;
	...
	/**
	 * The listener interface for receiving openButton events. The same
	 * interface is used for both the input and output directory options
	 *
	 */
	public class OpenButtonListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (sourceButton.equals(openButton)) // FIXME Change later to only
													// one if
			{
				sourcefc = fc;
			...
			if (returnVal == JFileChooser.APPROVE_OPTION) {
				...
				if (sourcefc.equals(fc)) {
					if (file.isDirectory())
						log.append("Chose Input Directory: " + inputDir
								+ newline);
					else
		...
		public void runMallet() {
			...
			if (inputDirTfield.getText().equals("")) {
				...
				try {
					...
					if (file.isDirectory())
						c = Class
								.forName("cc.mallet.classify.tui.Text2Vectors");
					else
					...
					for (int i = 0; i < advArgs.length; i = i + 2) {
						if (fieldOptionMap.containsKey(advArgs[i])) {
							if (fieldOptionMap.get(advArgs[i])[2]
									.equals("import")) {
								imp.add(advArgs[i]);
								imp.add(advArgs[i + 1]);
							} else if (fieldOptionMap.get(advArgs[i])[2]
									.equals("train")) {
								trn.add(advArgs[i]);
								trn.add(advArgs[i + 1]);
							}
						} else if (checkBoxOptionMap.containsKey(advArgs[i])) {
							if (checkBoxOptionMap.get(advArgs[i])[2]
									.equals("import")) {
								imp.add(advArgs[i]);
								imp.add(advArgs[i + 1]);
							} else if (checkBoxOptionMap.get(advArgs[i])[2]
									.equals("train")) {
								trn.add(advArgs[i]);
								trn.add(advArgs[i + 1]);
							}
						} else if (otherOptionsMap.containsKey(advArgs[i])) {
							if (otherOptionsMap.get(advArgs[i])
									.equals("import")) {
								imp.add(advArgs[i]);
								imp.add(advArgs[i + 1]);
							} else if (otherOptionsMap.get(advArgs[i]).equals(
									"train")) {
								trn.add(advArgs[i]);
								trn.add(advArgs[i + 1]);
							}
					...
					String[] temp1 = { "--input", inputDir, "--output",
							collectionPath, "--gram-sizes", numGrams.getText(),
							"--keep-sequence" };

					Collections.addAll(imp, temp1);
					String[] fullImportArgs = imp
							.toArray(new String[imp.size()]);
					...
					String stateFile = outputDir + File.separator
							+ "output_state.gz";
					String outputDocTopicsFile = outputDir + File.separator
							+ "output_doc_topics.txt";
					String topicKeysFile = outputDir + File.separator
							+ "output_topic_keys";
					...
					// System.out.println(passedArgs);
					// Now invoke the method.
					argTypes = new Class[] { fullTrainArgs.getClass(), }; // array
																			// is
																			// Object!
					...
					g.unzip(new File(outputDir + File.separator
							+ "output_state"));

					outputCsvFiles(outputDir, true);
					// outputHtmlFiles(outputDir);

					System.out.println("Mallet Output files written in "
							+ outputDir + " ---> " + stateFile + " , "
							+ topicKeysFile + newline);
					System.out.println("Csv Output files written in "
							+ outputDir + File.separator + "output_csv");
					System.out.println("Html Output files written in "
							+ outputDir + File.separator + "output_html");
				} catch (Throwable e1) {
					e1.printStackTrace();
				}
				log.setCaretPosition(log.getDocument().getLength());
	...
	public void setDefaultOptions() {
		...
		fieldOptionMap.put("--num-iterations", new String[] {
				"No. of iterations", "200", "train" });
		fieldOptionMap.put("--num-top-words", new String[] {
				"No. of topic words printed", "10", "train" });
		fieldOptionMap.put("--doc-topics-threshold", new String[] {
				"Topic proportion threshold", "0.05", "train" });
	...
	public void resetAdvControls() {
		...
		stopChooseButton.setEnabled(stopBox.isSelected()); // Not sure why this
															// doesn't happen
															// automatically
	...
	public String[] getAdvArgs() {
		...
		String[] advArgs = new String[(checkBoxOptionMap.size()
				+ fieldOptionMap.size() + 1) * 2]; // +1 for stopword list
		int index = 0;
		...
		for (String k : fieldOptionMap.keySet()) {
			...
			if (k.equals("--num-top-words")) {
				int topwords = Integer.parseInt(itr2.next().getText()) + 1;
	...
	public void buildAdvPanel() {
		...
		advCheckBoxPanel
				.setBorder(BorderFactory.createEmptyBorder(15, 0, 5, 5));
		...
		JPanel advFieldPanel = new JPanel(new GridLayout(fieldOptionMap.size(),
				2));
	...
	public void go() {
		...
		label_1 = new Label("n-grams");
		p3.add(label_1);

		numGrams = new JTextField(2);
		numGrams.setText("2");
		p3.add(numGrams);
		p3.add(new Label("Topics"));
		numTopics.setText("10");
		p3.add(numTopics);

		label = new Label("Theard:");
		p3.add(label);

		numThreads = new JTextField(2);
