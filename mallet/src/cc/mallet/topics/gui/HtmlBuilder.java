...
public class HtmlBuilder {
	...
	void createCss(File htmlDir, InputStream cssResource) throws IOException {
		...
		OutputStream out = new FileOutputStream(new File(htmlDir,
				"malletgui.css"));
	...
	void writeHtmlHeader(OutputStreamWriter b, String title, String cssPath) {
		String str = "<html><head><meta content=\"text/html; charset=ISO-8859-1\"http-equiv=\"Content-Type\"><head><link rel=\"stylesheet\" type=\"text/css\" href=\""
				+ cssPath + "\" />";
		try {
			b.write(str);
			writeTitle(b, title);
	...
	void writeHtmlFooter(OutputStreamWriter b) {
	...
	void writeReturnLink(OutputStreamWriter b) {
		String str = "<br><br><br><br>"
				+ makeUrl("../all_topics.html", "<b>[Index]</b>");
		try {
			b.write(str);
		} catch (IOException e) {
			// TODO Auto-generated catch block
	...
	void writeTitle(OutputStreamWriter b, String title) {
	...
	void writeFileExcerpt(File f, OutputStreamWriter out) throws IOException {
	...
	void writeLineExcerpt(String str, OutputStreamWriter out)
			throws IOException {
	...
	public void buildHtml1(File inputCsv, File outputDir) {
		try {
			...
			OutputStreamWriter out = new OutputStreamWriter(
					new FileOutputStream(outputDir + "/" + FILE_NAME), "UTF-8");
			out.write('\ufeff');
			...
			while ((line = in.readLine()) != null) {
				...
				out.write(String.format("<tr><td>%d. </td><td>%s</td></tr>", n,
						makeUrl("Topics/Topic" + st[0] + ".html", st[1])));
	...
	public void buildHtml2(File inputCsv, File outputDir) {
		try {
			...
			while ((line = in.readLine()) != null) {
				st = line.split(CSV_DEL);
				// values always have to be arranged pairwise -> if mod 2 not 0
				// this indicates prolems
				if (st.length % 2 != 0) {
					System.out.println(line);
				}
				String FILE_NAME = "Doc" + st[0] + ".html";
				OutputStreamWriter out = new OutputStreamWriter(
						new FileOutputStream(outputDir + "/" + FILE_NAME),
						"UTF-8");
				out.write('\ufeff');
				...
				if (input.isDirectory()) {
					...
					out.write("<body><h4><u>DOC</u> :" + df.getName()
							+ "</h4><br>");
					try {
					...
					out.write("<body><h4><u>DOC</u> : " + "doc " + st[0]
							+ "</h4><br>");
				...
				for (int i = 2; i < st.length - 1; i = i + 2) {
					try {
						out.write(String
								.format("<tr><td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td><td>(%.0f%%)</td><td>",
										new Float(st[i + 1]) * 100));
					} catch (Exception e) {
					}
					try {
						// create <a href=""> based on the values in the st
						// array
						out.write(makeUrl("../Topics/Topic" + st[i] + ".html",
								topics.get(Integer.parseInt(st[i]))));
					} catch (Exception e) {
						System.out.println();
					}
					out.write(" ...</td></tr>");
		...
		try {
			...
			while ((line = in.readLine()) != null) {
				...
				if (!st[0].equals(prevId)) {
					...
					out = new OutputStreamWriter(new FileOutputStream(outputDir
							+ "/" + FILE_NAME), "UTF-8");
					out.write('\ufeff');
					writeHtmlHeader(out, FILE_NAME, "../malletgui.css");
					// FIXME change to constant
					out.write("<body><h4><u>TOPIC</u> : "
							+ topics.get(Integer.parseInt(st[0])) + " ...</h4>");
					prevId = st[0];
				...
				String doc_name = makeUrl("../Docs/Doc" + st[st.length - 2]
						+ ".html ", tdocname);
				try {
					out.write(String
							.format("<tr><td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td><td>%d.</td><td>%s</td><td>%s</td></tr>",
									Integer.parseInt(st[1]) + 1,
									"("
											+ Ntd[Integer.parseInt(st[0])][Integer
													.parseInt(st[st.length - 2])]
											+ ")", doc_name));
				} catch (Exception e) {
				}
				// String temStr =
				// String.format("        %d. %6s    ","("+Ntd[Integer.parseInt(st[0])][Integer.parseInt(st[st.length-2])]+")");
				// out.write(temStr.replace(" ", "&nbsp;"));
				// out.write(makeUrl("../Docs/Doc"+st[st.length-2]+".html ",new
				// File(st[st.length-1]).getName()));
				// out.write(" <br>");
	...
	public void createHtmlFiles(File outputDir) {
		...
		try {
			createCss(htmlDir,
					TopicModelingTool.class
							.getResourceAsStream("malletgui.css"));
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
